...
function deleteFile(deleteFileName) {
	...
	if (confirm('<% = strTxtAreYouSureDeleteFile %> \'' + deleteFileName + '\'?')){
		self.location.href = 'file_delete.asp?fileName=' + escape(deleteFileName) + '<% If strMode = "A" Then Response.Write("&PF=" & lngUserProfileID & "&M=A") %><% = strQsSID2 %>';
		return true;
	}
}
<%
...
'If alloted space is 0 set the file upload to full (prevents divsion errors later)
If intUploadAllocatedSpace = 0 Then
