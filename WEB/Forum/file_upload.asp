...
If intUploadMaxImageSize >= 1024 Then 
	strMaxImageUpload = intUploadMaxImageSize / 1024 & " MB"
Else 
	strMaxImageUpload = intUploadMaxImageSize & " KB"
End If

'Calculate the file upload size in MB
If intUploadMaxFileSize >= 1024 Then 
	strMaxFileUpload = intUploadMaxFileSize / 1024 & " MB"
Else 
	strMaxFileUpload = intUploadMaxFileSize & " KB"
End If
...
%>    
   <div id="fileProperties"<% If Request.QueryString("T") <> "file" AND blnImageUpload Then Response.Write(" style=""display:none""") %>>          
    ...
    <br />
    <form method="post" action="file_upload.asp?PB=Y&T=file<% If strMode = "A" Then Response.Write("&PF=" & lngUserProfileID & "&M=A") %><% = strAspUploadPID %><% = strQsSID2 %>" name="frmUpload" enctype="multipart/form-data" onsubmit="return checkFile();" >
...
%>
   <div id="imageProperties"<% If Request.QueryString("T") = "file" Then Response.Write(" style=""display:none""") %>> 
    ...
    <br />
    <form method="post" action="file_upload.asp?PB=Y&T=image<% If strMode = "A" Then Response.Write("&PF=" & lngUserProfileID & "&M=A") %><% = strAspUploadPID %><% = strQsSID2 %>" name="frmUpload" enctype="multipart/form-data" onsubmit="return checkFile();" >
