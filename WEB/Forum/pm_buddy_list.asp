...
<form method="post" name="frmBuddy" id="frmBuddy" action="pm_add_buddy.asp<% = strQsSID1 %>" onSubmit="return CheckForm();" onReset="return ResetForm();">
  <table cellspacing="1" cellpadding="3" class="tableBorder" align="center">
    <tr class="tableLedger">
       ...
       <tr class="tableRow">
        <td>
          <input type="text" name="username" id="username" size="15" maxlength="25" value="<% If CInt(Request.QueryString("code")) <> 2 Then Response.Write(Server.HTMLEncode(decodeString(Request.QueryString("name")))) %>">
          <a href="javascript:winOpener('pop_up_member_search.asp?RP=BUD<% = strQsSID2 %>','memSearch',0,1,440,255)"><img src="<% = strImagePath %>member_search.<% = strForumImageType %>" alt="<% = strTxtMemberSearch %>" title="<% = strTxtMemberSearch %>" border="0" align="absmiddle"></a> 
         </td>
        <td> 
         <input type="text" name="description" id="description" size="25" maxlength="30" value="<% If CInt(Request.QueryString("code")) <> 2 Then Response.Write(Server.HTMLEncode(Request.QueryString("desc"))) %>">
        </td>
        <td> 
        ...
        <td width="21%" align="right"><input type="submit" name="Submit" id="Submit" value="<% = strTxtAddToBuddy %>"></td>
       </tr>
  </table>
</form>
<br />
...
Else 	
    ...
    <tr class="<% If (intRowColourNumber MOD 2 = 0 ) Then Response.Write("evenTableRow") Else Response.Write("oddTableRow") %>"> 
     ...
     <td align="center"><% If blnIsUserOnline Then Response.Write("<img src=""" & strImagePath & "yes.png"" alt=""" & strTxtOnLine2 & """ title=""" & strTxtOnLine2 & """ />") Else Response.Write("<img src=""" & strImagePath & "no.png"" alt=""" & strTxtOffLine & """ title=""" & strTxtOffLine & """ />") %></td><%
		...
		%>
     <td align="center"><a href="pm_delete_buddy.asp?pm_id=<% = sarryPmBuddy(1,intCurrentRecord) %><% = strQsSID2 %>" OnClick="return confirm('<% = strTxtDeleteBuddyAlert %>')"><img src="<% = strImagePath %>delete.<% = strForumImageType %>" width="15" height="16" alt="<% = strTxtDelete %>" title="<% = strTxtDelete %>" border="0"></a></td>
    </tr><%
