...
If blnAvatarUploadEnabled Then
	...
	If NOT rsCommon.EOF Then
		...
		strImageTypes = rsCommon("Upload_avatar_types")
		intUploadMaxImageSize = CInt(rsCommon("Upload_avatar_size"))
	End If
...
If lngErrorFileSize = 0 AND blnExtensionOK = True AND blnFileSpaceExceeded = False AND blnFileExists = False AND strImageName <> "" Then
	...
	Response.Write(vbCrLf & "	document.getElementById('fileName').innerHTML = '" & strImageName & "';")
...
function insertAvatar(){
	...
	if (document.getElementById('fileName').innerHTML == ''){
		...
		window.opener.document.getElementById('txtAvatar').value = '<% = strUploadFilePath  %>/' + document.getElementById('fileName').innerHTML;
		window.opener.document.getElementById('avatar').src = '<% = strUploadFilePath  %>/' + document.getElementById('fileName').innerHTML;
		window.close();
	}
}
    ...
    <tr>
     ...
     <td width="50%" valign="top"><form method="post" action="upload_avatars.asp?PB=Y<% = strQsSID2 %>" name="frmUpload" enctype="multipart/form-data" onsubmit="return checkFile();" >
       <table width="100%" border="0" cellspacing="0" cellpadding="1">
        ...
        <tr>
         <td><span class="smText"><% Response.Write(strTxtImagesMustBeOfTheType & ", " & Replace(strImageTypes, ";", ", ", 1, -1, 1) & ", " & strTxtAndHaveMaximumFileSizeOf & " " & intUploadMaxImageSize & "KB")  %></span></td>
        </tr>
        <tr>
      ...
      <table width="100%" border="0" cellspacing="1" cellpadding="0">
       <tr>
        ...
        <td width="75%"><span id="fileName"></span> <span id="fileDownload" style="display:none;"></span> <span id="fileSize" style="display:none;"></span></td>
       </tr>
       <tr>
