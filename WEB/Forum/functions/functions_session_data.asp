...
Private Sub getSessionData()
	...
	ElseIf Request.QueryString("SID") <> "" Then
		...
		strSessionID = LCase(Trim(Request("SID")))
	End If
	...
	If blnDatabaseHeldSessions Then
		...
		If strDatabaseType = "Access" Then
			strSQL = strSQL & "#" & internationalDateTime(DateAdd("n", -intSessionTimeout, Now())) & "#;"
		ElseIf strDatabaseType = "SQLServer" Then
			strDate = internationalDateTime(DateAdd("n", -intSessionTimeout, Now()))
...
Private Function getSessionItem(ByVal strSessionKey)
	...
	If NOT strOSType = "Search Robot" Then
	...
	If NOT strOSType = "Search Robot" Then
		...
		For intSessionArrayPass = 0 To UBound(sarySessionData, 2)
			...
			If sarySessionData(0, intSessionArrayPass) = strSessionID Then
				...
				If blnDatabaseHeldSessions AND NOT strOSType = "Search Robot" Then
